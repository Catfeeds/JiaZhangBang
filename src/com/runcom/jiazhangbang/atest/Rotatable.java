...
/**
 * 旋转工具类 Created by Yahya Bayramoglu on 01/12/15.
 */
public class Rotatable implements View.OnTouchListener
{
	...
	/**
	 * This method needs to be call, if only you need to reset and rebuild a
	 * view as rotatable with different configurations
	 */
	public void drop()
	{
	...
	/**
	 * You may need to enable / disable touch interaction at some point, so it
	 * is possible to do it so anytime by rotatable object
	 */
	...
	/**
	 * If your application can be used multi orientated, then you have to
	 * declare orientation changes to rotatable object, so it can recalculate
	 * its maxDistances.
	 * <p>
	 * You only need to inform rotatable object about orientation changes, when
	 * you specified {@link Builder#rotationCount(float)} or
	 * {@link Builder#rotationDistance(float)}
	 */
	...
	public void takeAttention()
	{
		...
		set.addListener(new AnimatorListenerAdapter()
		{
			...
			public void onAnimationEnd(Animator animation )
			{
				...
				rootView.animate().rotationX(0).rotationY(0).setDuration(FIT_ANIM_TIME).setInterpolator(new FastOutSlowInInterpolator()).start();
			}
		});
	...
	/**
	 * Animate rotatable object with given direction and degree also possible to
	 * set duration and a listener with other derivation of this method
	 */
	...
	private void measureScreenUpToOrientation(int screenOrientation )
	{
		...
		{
			/**
			 * If screenOrientation is landscape, then width will have a larger
			 * value than height
			 */
		...
		{
			/**
			 * If screenOrientation: is portrait, then width will have a smaller
			 * value than height is square, then width and height will be same
			 * is unknown, then unknown to rotatable as well so either way...
			 */
	...
	{
		return ( -270 >= value && value >= -360) || ( -90 <= value && value <= 90) || (270 <= value && value <= 360);
	}
	...
	private void swapViews(int rotation )
	{
		...
		if(rotation == ROTATE_BOTH)
		{
			...
			|| (currentXRotation > -90 && currentXRotation < 90) && (currentYRotation > -360 && currentYRotation < -270) || (currentXRotation > -360 && currentXRotation < -270) && (currentYRotation > -90 && currentYRotation < 90)

			|| (currentXRotation > -90 && currentXRotation < 90) && (currentYRotation > 270 && currentYRotation < 360) || (currentXRotation > 270 && currentXRotation < 360) && (currentYRotation > -90 && currentYRotation < 90)

			|| (currentXRotation > 90 && currentXRotation < 270) && (currentYRotation > -270 && currentYRotation < -90) || (currentXRotation > -270 && currentXRotation < -90) && (currentYRotation > 90 && currentYRotation < 270)

			|| (currentXRotation > 90 && currentXRotation < 270) && (currentYRotation > 90 && currentYRotation < 270) || (currentXRotation > -270 && currentXRotation < -90) && (currentYRotation > -270 && currentYRotation < -90);
		}
	...
	public static class Builder
	{
		...
		/**
		 * Declaring sides will provide swapping between them when necessary, if
		 * not declared, then rootView will be rotating by itself without any
		 * other effect
		 */
		...
		/**
		 * This method provides view to rotate once in given distance, note that
		 * it won't rotate full if touch distance is not enough but it may still
		 * fit the rotation. If you want to ensure rotation gets completed see
		 * {@link #rotationCount(float}
		 */
		...
		/**
		 * Consider not to change pivot values because view may out of its
		 * bounders and get invisible.
		 */
